...
def get_statistics(filename, top_k_concepts):
    ...
    k = top_k_concepts
    sorted_concepts_desc = np.argsort(
        list(dict_concept_counter.values()))[::-1][:k]
    sorted_concepts_asc = np.argsort(list(dict_concept_counter.values()))[:k]

    most_frequent_concepts = [list(dict_concept_counter.keys())[
        i] for i in sorted_concepts_desc]
    less_frequent_concepts = [list(dict_concept_counter.keys())[
        i] for i in sorted_concepts_asc]
...
def plot_bar_chart(list_cuis, list_of_concepts_per_image, title):
    # Plot chart
    bars = plt.bar(np.arange(0, max(list_cuis) + 1, 1),
                   list_of_concepts_per_image)
    plt.bar_label(bars)
    plt.title(title)
...
def compute_pos_weights(dataset_csv, concept_csv):
    ...
    np.testing.assert_array_equal(np.ones_like(
        pos_count) * N, np.sum((neg_count, pos_count), axis=0))
...
# Run to test some code
if __name__ == '__main__':
    dataset_csv = "data/csv/concepts/top100/new_train_subset_top100.csv"
    concepts_csv = "data/csv/concepts/top100/new_top100_concepts.csv"

    out = compute_pos_weights(dataset_csv, concepts_csv)
    print(out)

    out2 = get_class_weights(dataset_csv, concepts_csv, 100)
    print(out2)
