    ...
    def compute_loss(self, model, input, return_outputs=False):
        ...
        with torch.no_grad():
            baseline = model.greedy_search(
                **input, stopping_criteria=self.stopping_criteria,logits_processor=self.logits_processor, logits_warper=self.logits_warper)
            baseline = baseline[:, 1:]  # ignore BOS token (since labels does not include it)
