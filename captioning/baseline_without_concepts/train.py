...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--logdir",
        type=str,
        default="/media/TOSHIBA6T/ICC2022/captioning/baseline_without_concepts/",
    ...
    parser.add_argument(
        ...
        choices=["microsoft/beit-base-patch16-224-pt22k-ft22k", "google/vit-base-patch16-224", "facebook/deit-tiny-patch16-224"],
    ...
    # Training
    parser.add_argument(
        "--scst", action="store_true", help="Do Self-Critical Sequence Training."
    )
    parser.add_argument(
        "--ckpt", type=str, default=None, help="Resume training from this checkpoint."
    )
    ...
    if(args.ckpt):
        # load from trained model checkpoint
        ...
        if "gpt2" in args.decoder:
            ...
            special_tokens_dict = {'additional_special_tokens': ["<|startoftext|>", "<|pad|>"]}
        ...
        feature_extractor = AutoFeatureExtractor.from_pretrained(args.encoder)
    ...
    training_args = Seq2SeqTrainingArguments(
        ...
        log_level="debug",
        #ignore_data_skip=True, # TO REMOVE LATER
    )

    if(args.scst):
        trainer = CustomTrainer(
            model=model,
            tokenizer=tokenizer, # to ensure it is saved (useful later for loading during generation)
