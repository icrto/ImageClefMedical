...
for subset in [test_path, validation_path]:
    ...
    assert len(csv_files) == 9, "Check you directory."
    ...
    for csv in csv_files:
        ...
        for index, row in df.iterrows():
            ...
            if subset == "validation":
                ...
                eval_data[row["ID"]] = list(dict.fromkeys(eval_data[row["ID"]]))
    ...
    if subset == "validation":
        ...
        predictions_df = evaluation_df.copy().merge(valid_df.copy(), on="ID", suffixes=('_pred', '_gt'))
        ...
        concept_dict_name_to_idx, concept_dict_idx_to_name, _ = get_concepts_dicts(data_dir=sem_concepts_path, concepts_csv="new_top100_concepts_sem.csv")
        valid_csvpath = os.path.join(data_dir, "top100", "new_val_subset_top100_sem.csv")
